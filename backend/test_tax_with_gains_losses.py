...
def create_portfolio_with_gains_and_losses() -> Portfolio:
    """Create a test portfolio with both gains and losses"""
    stocks = [
        Stock('AAPL', 20, 100.0, date(2023, 1, 1)),   # Long-term, gain (selling at $150)
        Stock('GOOGL', 10, 2500.0, date(2023, 6, 1)), # Long-term, loss (selling at $1000)
        Stock('TSLA', 5, 300.0, date(2024, 10, 1)),   # Short-term, gain (selling at $800)
        Stock('AMZN', 8, 200.0, date(2024, 11, 1))    # Short-term, loss (selling at $120)
    ]
    cash = 5000.0
    return Portfolio(stocks=stocks, cash=cash)


def test_tax_with_gains_and_losses():
    """Test tax calculation with both gains and losses"""
    
    analyzer = TradePaymentAnalyzer()
    portfolio = create_portfolio_with_gains_and_losses()
    
    print("=" * 60)
    print("TEST: Trade with gains and losses - tax implications")
    print("=" * 60)
    
    # Create a trade that requires selling assets
    trade = Trade('NVDA', 25, 800.0, 'buy')
    ...
    try:
        ...
        if result['assets_sold']:
            ...
            for asset in result['assets_sold']:
                ...
                # Calculate profit/loss manually for display
                original_stock = next(s for s in portfolio.stocks if s.symbol == asset['symbol'])
                profit_per_share = asset['sale_price_per_share'] - original_stock.avg_price
                total_profit = profit_per_share * asset['shares_sold']
                print(f"  Profit/Loss per share: ${profit_per_share:.2f}")
...
if __name__ == "__main__":
    test_tax_with_gains_and_losses()
