...
def create_portfolio_with_actual_gains() -> Portfolio:
    """Create a test portfolio with actual gains (selling price > purchase price)"""
    stocks = [
        Stock('AAPL', 20, 100.0, date(2023, 1, 1)),   # Long-term, gain
        Stock('GOOGL', 10, 800.0, date(2023, 6, 1)),  # Long-term, gain
        Stock('TSLA', 5, 200.0, date(2024, 10, 1)),   # Short-term, gain
        Stock('AMZN', 8, 80.0, date(2024, 11, 1))     # Short-term, gain
    ]
    cash = 2000.0
    return Portfolio(stocks=stocks, cash=cash)


def test_actual_tax_calculation():
    """Test tax calculation with actual gains (selling at current market prices)"""
    
    analyzer = TradePaymentAnalyzer()
    portfolio = create_portfolio_with_actual_gains()
    
    print("=" * 60)
    print("TEST: Trade with actual gains - tax implications")
    print("=" * 60)
    
    # Create a trade that requires selling assets
    trade = Trade('NVDA', 15, 800.0, 'buy')
    ...
    try:
        ...
        # Show detailed tax calculations with actual market prices
        if result['assets_sold']:
            print("\nDetailed Tax Calculations:")
            current_prices = {'AAPL': 180.0, 'GOOGL': 150.0, 'TSLA': 250.0, 'AMZN': 170.0}
            
            for asset in result['assets_sold']:
                original_stock = next(s for s in portfolio.stocks if s.symbol == asset['symbol'])
                current_price = current_prices.get(asset['symbol'], asset['sale_price_per_share'])
                
                print(f"\n{asset['symbol']}:")
                print(f"  Shares sold: {asset['shares_sold']}")
                print(f"  Purchase price: ${original_stock.avg_price:.2f}")
                print(f"  Current market price: ${current_price:.2f}")
                print(f"  Days held: {asset['days_held']}")
                
                # Recalculate tax based on actual market price
                profit_per_share = current_price - original_stock.avg_price
                total_profit = profit_per_share * asset['shares_sold']
                
                if total_profit > 0:
                    tax_rate = 0.15 if asset['days_held'] > 365 else 0.25
                    actual_tax = total_profit * tax_rate
                    print(f"  Profit per share: ${profit_per_share:.2f}")
                    print(f"  Total profit: ${total_profit:.2f}")
                    print(f"  Tax rate: {tax_rate:.2f} ({'long-term' if asset['days_held'] > 365 else 'short-term'})")
                    print(f"  Actual tax would be: ${actual_tax:.2f}")
                else:
                    print(f"  Loss per share: ${abs(profit_per_share):.2f}")
                    print(f"  Total loss: ${abs(total_profit):.2f}")
                    print(f"  Tax: $0.00 (no tax on losses)")
